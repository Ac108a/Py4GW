...
# --- Minimal consumable counting (no duplicate lists) -----------------------
def _ids_for(key: str):
    """Return list[int model_id] using existing sources only."""
    ids = []
    try:
        # families (defined once in backend)
        if key in ("Alcohol", "Morale", "CitySpeed"):
            fam = getattr(Yield.Upkeepers, f"{key.upper()}_ITEMS", [])
            ids = [int(getattr(x, "value", x)) for x in fam]
            return ids
    except Exception:
        pass
    # singles: use the already-present ICON_MODEL in the selector
    try:
        from Bots.aC_Scripts.Consumable import ConsumablesSelectorFull as CSF
        mid = CSF.ICON_MODEL.get(key)
        if mid is not None:
            ids = [int(getattr(mid, "value", mid))]
    except Exception:
        ids = []
    return ids

def _bag_count(model_ids):
    return sum(int(GLOBAL_CACHE.Inventory.GetModelCount(int(mid)) or 0) for mid in (model_ids or []))

def _record_used(label: str, used: int):
    if used <= 0: 
        return
    try:
        from PyQuishAi.runner_singleton import runner_fsm
        ...
        if cs:
            cs.record_consumable(label, int(used))
    except Exception:
        pass

# ---- Consumable baseline (for diff display only) ---------------------------
_consumable_baseline: Dict[str, int] = {}

def _cons_reset_baseline():
    """Snapshot current bag counts per key using existing IDs only."""
    global _consumable_baseline
    _consumable_baseline = {
        key: _bag_count(_ids_for(key))
        for key in ConsumablesSelectorFull.consumable_state.keys()
    }

def _cons_used_since_baseline(key: str) -> int:
    """Return max(0, baseline - current) for a single key; lazily initializes."""
    global _consumable_baseline
    if not _consumable_baseline:     # baseline not captured yet
        _cons_reset_baseline()
    base = _consumable_baseline.get(key, 0)
    cur  = _bag_count(_ids_for(key))
    used = base - cur
    return used if used > 0 else 0

def _title_points_now(title_id: int) -> int:
    try:
        t = Player.GetTitle(title_id)
        return int(getattr(t, "current_points", 0) or 0)
    except Exception:
        return 0
...
class ConsumablesHelper:
    def __init__(self):
        self.started = False
        # key, upkeep, args, kwargs
        self._table = [
            ("Cupcake",            Routines.Yield.Upkeepers.Upkeep_BirthdayCupcake,     (), {}),
            ("Alcohol",            Routines.Yield.Upkeepers.Upkeep_Alcohol,             (), {"target_alc_level": 1, "disable_drunk_effects": True}),
            ("Morale",             Routines.Yield.Upkeepers.Upkeep_Morale,              (110,), {}),

            ("CitySpeed",          Routines.Yield.Upkeepers.Upkeep_City_Speed,          (), {}),

            ("ArmorOfSalvation",   Routines.Yield.Upkeepers.Upkeep_ArmorOfSalvation,    (), {}),
            ("EssenceOfCelerity",  Routines.Yield.Upkeepers.Upkeep_EssenceOfCelerity,   (), {}),
            ("GrailOfMight",       Routines.Yield.Upkeepers.Upkeep_GrailOfMight,        (), {}),

            ("BlueRockCandy",      Routines.Yield.Upkeepers.Upkeep_BlueRockCandy,       (), {}),
            ("GreenRockCandy",     Routines.Yield.Upkeepers.Upkeep_GreenRockCandy,      (), {}),
            ("RedRockCandy",       Routines.Yield.Upkeepers.Upkeep_RedRockCandy,        (), {}),

            ("SliceOfPumpkinPie",  Routines.Yield.Upkeepers.Upkeep_SliceOfPumpkinPie,   (), {}),
            ("BowlOfSkalefinSoup", Routines.Yield.Upkeepers.Upkeep_BowlOfSkalefinSoup,  (), {}),
            ("CandyApple",         Routines.Yield.Upkeepers.Upkeep_CandyApple,          (), {}),
            ("CandyCorn",          Routines.Yield.Upkeepers.Upkeep_CandyCorn,           (), {}),
            ("DrakeKabob",         Routines.Yield.Upkeepers.Upkeep_DrakeKabob,          (), {}),
            ("GoldenEgg",          Routines.Yield.Upkeepers.Upkeep_GoldenEgg,           (), {}),
            ("PahnaiSalad",        Routines.Yield.Upkeepers.Upkeep_PahnaiSalad,         (), {}),
            ("WarSupplies",        Routines.Yield.Upkeepers.Upkeep_WarSupplies,         (), {}),
        ]
    ...
    def run(self):
        while True:
            ...
            for key, fn, args, kwargs in self._table:
                if not s.get(key, False):
                    continue

                ids = _ids_for(key)
                before = _bag_count(ids)

                # call the real Upkeeper
                yield from fn(*args, **kwargs)

                after = _bag_count(ids)
                _record_used(key, max(0, before - after))
...
def DrawWindow():
    ...
    fsm_active = bool(runner_fsm.map_chain)
    ...
    state_name = ""
    try:
        state_name = runner_fsm.get_ui_state()
    except Exception:
        pass
    ...
    if PyImGui.small_button("Open Statistics"):
    ...
    if PyImGui.is_item_hovered():
        ...
        PyImGui.text("When enabled, after each run the bot donates Luxon/Kurzick if unspent is > threshold.")
...
def draw_statistics_window():
    ...
    cs = getattr(runner_fsm, "chain_stats", None)
    if not cs:
        PyImGui.text("No statistics captured yet.")
        if PyImGui.small_button("Reset statistics"):
            try:
                runner_fsm.reset_statistics()
            except Exception:
                from PyQuishAi.StatsManager import ChainStatistics
                runner_fsm.chain_stats = ChainStatistics(runner_fsm.map_chain or [])
                runner_fsm.chain_stats.start_chain()
        PyImGui.end()
        return
    ...
    PyImGui.pop_style_color(1)
    lux = int(cs.donations.get("luxon", 0) or 0)
    kur = int(cs.donations.get("kurzick", 0) or 0)
    tot = int(cs.total_faction_donated or 0)
    PyImGui.text(f"Kurzick donated: {kur:,}")
    PyImGui.text(f"Luxon donated:   {lux:,}")
    PyImGui.text(f"Total Donation x 8: {tot * 8:,}")   # <— multiplied by 8
    PyImGui.separator()

    # Title Points (show session gains; snapshot baseline once)
    PyImGui.push_style_color(PyImGui.ImGuiCol.Text, (0.85, 0.75, 0.3, 1.0))
    PyImGui.text("Title Points")
    PyImGui.pop_style_color(1)
    ...
    # Snapshot baseline once per stats session
    if not hasattr(cs, "title_points_baseline"):
        cs.title_points_baseline = {tid: _title_points_now(tid) for _, tid in title_ids}

    shown_any_title = False
    for name, tid in title_ids:
        cur  = _title_points_now(tid)
        base = cs.title_points_baseline.get(tid, 0)
        diff = max(0, cur - base)
        if diff > 0:
            PyImGui.text(f"{name}: +{diff:,}")
            shown_any_title = True
    if not shown_any_title:
        PyImGui.text("—")

    # Per-map (de-duplicated by map id, in chain order)
    PyImGui.push_style_color(PyImGui.ImGuiCol.Text, (0.85, 0.75, 0.3, 1.0))
    PyImGui.text("Per Map")
    PyImGui.pop_style_color(1)

    seen = set()
    # Prefer the *declared chain order*, but this still works after loops
    for run in sorted(cs.runs, key=lambda r: r.order):
        if run.id in seen:
            continue
        seen.add(run.id)
        p = cs.per_map.get(run.id)
        if not p:
            continue
        label = run.display or run.id
        avg_sec = (p["total_time"] / p["runs"]) if p["runs"] else 0
        PyImGui.text(label)
        PyImGui.text(
            f"Success:{p['success']}  Fails:{p['fails']}  "
            f"Vanquished:{p['vanquished']} / Not:{p['not_vanquished']}  "
            f"Avg:{_fmt_hms(avg_sec)}"
        )

    PyImGui.separator()

    # Consumables diff (unchanged)
    PyImGui.push_style_color(PyImGui.ImGuiCol.Text, (0.85, 0.75, 0.3, 1.0))
    PyImGui.text("Consumables Used (since baseline)")
    PyImGui.pop_style_color(1)

    PyImGui.same_line(0, 8)
    if PyImGui.small_button("Reset baseline"):
        _cons_reset_baseline()

    shown_any = False
    for key in ConsumablesSelectorFull.consumable_state.keys():
        used = _cons_used_since_baseline(key)
        if used > 0:
            shown_any = True
            PyImGui.text(f"{key}: {used:,}")
    if not shown_any:
        PyImGui.text("—")
